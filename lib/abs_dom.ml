...
module Abs_Loc = struct
  type t =
    | Bot
    | AVarLoc of { id : Var.t; offset : Itv.t }
    | AHeapLoc of { lbl : Exp.Lbl.t; offset : Itv.t }
    | Top
  ...
  let join l1 l2 =
    ...
    | Top, _ | _, Top -> Top
    | AVarLoc { id = id1; offset = off1 }, AVarLoc { id = id2; offset = off2 }
      when Var.compare id1 id2 = 0 ->
        AVarLoc { id = id1; offset = Itv.join off1 off2 }
    | ( AHeapLoc { lbl = lbl1; offset = off1 },
        AHeapLoc { lbl = lbl2; offset = off2 } )
      when Exp.Lbl.compare lbl1 lbl2 = 0 ->
...
module Abs_Env = struct
  ...
  let find (e : t) (x : string) : Abs_Loc.t option = Var.Map.find_opt x e
  let write (e : t) (x : string) (l : Abs_Loc.t) : t = Var.Map.add x l e
...
module Error = struct
  ...
  let make ~(at : ProgramPoint.t) ~(access : access) ~(base : Abs_Loc.t)
      (* ~(offset:Itv.t) *)
      ~(in_itv : Itv.t) ~(left_oob : Itv.t) ~(right_oob : Itv.t)
      ~(base_pp : PPSet.t) ~(offset_pp : PPSet.t) ~(handler_caused : bool) : t =
    {
      at;
      access;
      base;
    ...
    if c <> 0 then c
      ...
      if c <> 0 then c
        ...
        if c <> 0 then c
          ...
          (* let c = Itv.compare e1.offset e2.offset in if c <> 0 then c else *)
            ...
            if c <> 0 then c
              ...
              if c <> 0 then c
                ...
                if c <> 0 then c
                  ...
                  if c <> 0 then c
                  else Bool.compare e1.handler_caused e2.handler_caused
