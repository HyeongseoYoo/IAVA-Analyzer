...
module Exp = struct
  ...
  and  t =
    ...
    module Lbl_map = struct
      ...
      let string_of_t : key -> string = function
        | Either.Left lbl -> Printf.sprintf "[L]%s" (Lbl.string_of_t lbl) (* normal [L]abel *)
        | Either.Right glbl -> Printf.sprintf "[G]%s" (Lbl.string_of_t glbl) (* [G]host label *)
    end
  ...
  let tabulate (l_exp: lbl_t) : t Lbl_map.t =
    let rec tabulate' ({lbl; exp}: lbl_t) (tbl : t Lbl_map.t) =
      ...
      | While (glbl, e1, e2) -> tbl |> Lbl_map.add (Either.Right glbl) exp |> tabulate' e1 |> tabulate' e2
    in
  ...
  and string_of_t ?(lvl : int = 0) : t -> string = function
    ...
    | Malloc (e1, e2) ->
        Printf.sprintf "malloc(%s, %s)" (string_of_lbl_t e1) (string_of_lbl_t e2)
    | Assign (e1, e2) ->
        Printf.sprintf "%s := %s" (string_of_lbl_t e1) (string_of_lbl_t e2)
    | Seq (e1, e2) -> Printf.sprintf "%s;\n%s" (string_of_lbl_t e1) (string_of_lbl_t e2)
    | If (e1, e2, e3) -> Printf.sprintf "if %s\nthen %s\nelse %s" (string_of_lbl_t e1) (string_of_lbl_t e2) (string_of_lbl_t e3)
    | While (_, e1, e2) ->
        Printf.sprintf "while %s\ndo %s" (string_of_lbl_t e1) (string_of_lbl_t e2)
    | Let (x, e1, e2) ->
        Printf.sprintf "let %s = %s in\n%s" x (string_of_lbl_t e1) (string_of_lbl_t e2)
end
...
module Program = struct
  ...
  let string_of_t (p : t) : string =
    Printf.sprintf "init {\n%s \n%s \n}\n\nmain {\n%s\n}\n"
      (Exp.string_of_lbl_t p.global) (Handler.string_of_handler_list p.handler) (Exp.string_of_lbl_t p.main)
end