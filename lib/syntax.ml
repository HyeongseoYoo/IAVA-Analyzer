...
module Exp = struct
  ...
  let rec string_of_t : t -> string = function
    ...
    | Disable -> "disable"
    | Bop (bop, e1, e2) ->
        Printf.sprintf "(%s %s %s)" (string_of_t e1) (string_of_bop bop)
          (string_of_t e2)
    | Deref (e1, e2) ->
        Printf.sprintf "*%s[%s]" (string_of_t e1) (string_of_t e2)
    | Malloc (e1, e2) ->
        Printf.sprintf "malloc(%s, %s)" (string_of_t e1) (string_of_t e2)
    | Assign (e1, e2) ->
        Printf.sprintf "%s := %s" (string_of_t e1) (string_of_t e2)
    | Seq (e1, e2) -> Printf.sprintf "%s; %s" (string_of_t e1) (string_of_t e2)
    | If (e1, e2, e3) ->
        Printf.sprintf "if %s then %s else %s" (string_of_t e1) (string_of_t e2)
          (string_of_t e3)
    | While (e1, e2) ->
        Printf.sprintf "while %s do %s" (string_of_t e1) (string_of_t e2)
    | Let (x, e1, e2) ->
        Printf.sprintf "let %s = %s in %s" x (string_of_t e1) (string_of_t e2)
end
...
module Init = struct
  ...
  let string_of_t (i : t) : string =
    ...
    | hs ->
        let hs_s =
          hs |> List.map Handler.string_of_def |> String.concat ";\n  "
        in
        Printf.sprintf "%s;\n  %s" globals_s hs_s
end
...
module Program = struct
  ...
  let string_of_t (p : t) : string =
    Printf.sprintf "init {\n  %s\n}\n\nmain {\n  %s\n}\n"
      (Init.string_of_t p.init) (Exp.string_of_t p.main)
end
...
module Tabulate = struct
  ...
  let tabulate (mkkey : int -> Label.t) (e : Exp.t) (tbl : t) : t =
    let rec go (n : int) (e : Exp.t) (tbl : t) : int * t =
      ...
      let n' = n + 1 in
      match e with
      | Exp.Unit | Exp.Int _ | Exp.Var _ | Exp.Enable | Exp.Disable -> (n', tbl)
      | Exp.Bop (_, e1, e2)
      | Exp.Malloc (e1, e2)
      | Exp.Assign (e1, e2)
      | Exp.Seq (e1, e2)
  ...
  let tabulate_all (p : Program.t) : t =
    ...
    let tbl2 =
      List.fold_left
        ...
        tbl1 p.init.handlers
    in
    (* main *)
    tabulate (fun n -> Label.Main n) p.main tbl2

  let iter (f : Label.t -> Exp.t -> unit) (tbl : t) : unit = Tbl.iter f tbl
end
