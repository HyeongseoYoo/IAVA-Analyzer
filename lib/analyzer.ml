...
let rec eval ?(lvalue=false) (c: conf) (lbl_exp: Exp.lbl_t)  : (result * conf) =
  ...
  | Var x -> (
    ...
    | None -> ({r with value = Value.Loc fresh_loc}, {c with env = Var.Map.add x fresh_loc env; fresh_loc = fresh_loc + 1})
    )
    else (
      match Var.Map.find_opt x env with
      | Some l -> (
  ...
  | Malloc (e1, e2) -> (
    ...
    let n = (match r1.value with 
      | Value.Int n' -> n' 
      | _ -> failwith "Malloc size must be an integer") in
    let v = r2.value in
    let mem' = c2.mem in
    let new_v = (v, ProgramPoint.Label lbl) in
    let mem'' = (List.init n (fun i -> c2.fresh_loc + i)
      |> List.fold_left
           (fun m a -> Loc.Map.add a new_v m)
           mem') in
    ({r with value = Value.Loc c2.fresh_loc}, {c with mem = mem''; fresh_loc = c2.fresh_loc + n})
    )
  | Deref (e1, e2) -> (
    let (r1, c1) = eval c e1 in
    let (r2, c2) = eval c1 e2 in
    let base = (match r1.value with
      | Value.Loc l -> l
      | _ -> failwith "Deref base must be a location") in
    let offset = (match r2.value with
      | Value.Int i -> i
      | _ -> failwith "Deref offset must be an integer") in
    let l = base + offset in
    if lvalue then (
      ({r with value = Value.Loc l}, c2)
    )
    else (
  ...
  | Assign (e1, e2) -> (
    ...
    let (r2, c2) = eval c1 e2 in
    let l = (match r1.value with
      | Value.Loc l -> l
      | _ -> failwith "Left-hand side of assignment must be a location") in
    Printf.printf "[Assign] %s := <%s, %s>\n" (Loc.string_of_t l) (Value.string_of_t r2.value) (ProgramPoint.string_of_t (ProgramPoint.Label lbl));
  ...
  | Let (x, e1, e2) -> (
    ...
    let c2 = {c1 with env = env'; mem = mem'; fresh_loc = c1.fresh_loc + 1} in
